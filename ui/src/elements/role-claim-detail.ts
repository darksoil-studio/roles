...
export class RoleClaimDetail extends SignalWatcher(LitElement) {
  ...
  async deleteRoleClaim() {
    try {
      ...
      this.dispatchEvent(new CustomEvent('role-claim-deleted', {
        bubbles: true,
        composed: true,
        detail: {
  ...
  renderDetail(entryRecord: EntryRecord<RoleClaim>) {
    return html`
      <sl-card>
          ...
          <sl-icon-button .src=${wrapPathInSvg(mdiDelete)} @click=${() => this.deleteRoleClaim()}></sl-icon-button>
        </div>
        ...
        <div class="column" style="gap: 16px;">
          ...
          <div class="column" style="gap: 8px;">
	        <span><strong>${msg("Role Name")}</strong></span>
 	        <span style="white-space: pre-line">${ entryRecord.entry.role_name }</span>
	  </div>
  ...
  render() {
    const roleClaim = this.rolesStore.roleClaims.get(this.roleClaimHash).entry.get();
