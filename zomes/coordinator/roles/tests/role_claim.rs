...
mod common;
use common::{create_role_claim, sample_role_claim_1, sample_role_claim_2};


#[tokio::test(flavor = "multi_thread")]
async fn create_role_claim_test() {
    ...
    let entry: RoleClaim = record.entry().to_app_option().unwrap().unwrap();
    assert!(entry.eq(&sample));
}


#[tokio::test(flavor = "multi_thread")]
async fn create_and_read_role_claim() {
    // Use prebuilt dna file
    let dna_path = std::env::current_dir()
        .unwrap()
        .join(std::env::var("DNA_PATH").expect("DNA_PATH not set, must be run using nix flake check"));
    ...
    let get_record: Option<Record> = conductors[1]
        .call(&bob_zome, "get_role_claim", record.signed_action.action_address().clone())
        .await;
...
#[tokio::test(flavor = "multi_thread")]
async fn create_and_delete_role_claim() {
    // Use prebuilt dna file
    let dna_path = std::env::current_dir()
        .unwrap()
        .join(std::env::var("DNA_PATH").expect("DNA_PATH not set, must be run using nix flake check"));
    ...
    let sample_1 = sample_role_claim_1(&conductors[0], &alice_zome).await;
    
    // Alice creates a RoleClaim
    let record: Record = create_role_claim(&conductors[0], &alice_zome, sample_1.clone()).await;
    ...
    let delete_action_hash: ActionHash = conductors[0]
        .call(&alice_zome, "delete_role_claim", original_action_hash.clone())
        .await;
    ...
    let deletes: Vec<SignedActionHashed> = conductors[1]
        .call(&bob_zome, "get_all_deletes_for_role_claim", original_action_hash.clone())
        .await;
